...
function Tonnetz2D({ selectedNotes, onSelectNote, theme = 'dark', cols = 8, rows = 6 }) {
  ...
  function getTriangles() {
    ...
    for (let col = 0; col < cols; col++) {
      for (let row = 0; row < rows; row++) {
        // Each node can be the top of two triangles (down-right, down-left)
        // Down-right triangle
  ...
  useEffect(() => {
    ...
    // True inscribed hex grid calculation with perfect centering and aspect ratio
    const gridWidthUnits = (cols - 1) * 0.75 + 1;
    const gridHeightUnits = (rows - 1) * Math.sqrt(3) / 2 + 1;
    // Margin for node radius on all sides
    // Let hexRadius be r, so total width needed is gridWidthUnits*r + 2*r
    // => r = min(width / (gridWidthUnits + 2), height / (gridHeightUnits + 2))
    ...
    if (selectedNotes.length === 3) {
      ...
      for (const tri of triangles) {
        // Check if selectedNotes match this triangle (order-insensitive)
    ...
    for (let col = 0; col < cols; col++) {
      for (let row = 0; row < rows; row++) {
        ...
        const y = y0 + row * hexRadius * Math.sqrt(3) / 2 + (col % 2) * hexRadius * Math.sqrt(3) / 4;
        ctx.beginPath();
        ctx.arc(x, y, hexRadius * 0.38, 0, 2 * Math.PI);
        ctx.fillStyle = selectedNotes.includes(note) ? nodeSelected : nodeColor;
  ...
  function handleClick(e) {
    ...
    // Dynamic layout (must match above)
    ...
    for (let col = 0; col < cols; col++) {
      for (let row = 0; row < rows; row++) {
        ...
        const dx = x - cx;
        const dy = y - cy;
        if (dx * dx + dy * dy < (hexRadius * 0.38) ** 2) {
  ...
  return (
    <div className="card p-0 text-center rounded-xl shadow-lg bg-card relative min-h-[500px] h-[60vh] max-h-[70vh] flex items-center justify-center">
      <canvas
        ref={canvasRef}
      ...
      />
      <div className="absolute top-2 left-2 text-xs text-text-secondary z-10">
        Click on nodes to select notes
      </div>
    </div>
  );
} 